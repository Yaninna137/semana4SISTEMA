...
class VentanaGrafica(QWidget):
    def __init__(self, matriz_obj):
        super().__init__()
        self.setWindowTitle("Programación Lineal - Gráfica")
    ...
    def graficar(self):
        ...
        # --- L1 y L2: ejes base ---
        ax.axhline(0, color='black', linewidth=1.5, label="L1: y = 0")
        ax.axvline(0, color='black', linewidth=1.5, label="L2: x = 0")

        # --- Graficar restricciones ---
        for i, fila in enumerate(restricciones):
            a, b, c = fila
            nombre = f"L{i + 3}"  # porque L1 y L2 ya son ejes
            ...
            if b != 0:
                ...
                x_const = c / a
                ax.axvline(x=x_const, linestyle='--', label=nombre)

        # --- Graficar vértices ---
        for i, v in enumerate(vertices, start=1):
            ax.scatter(v["x"], v["y"], s=60)
            ax.text(v["x"] + 0.1, v["y"] + 0.1, f"V{i}({v['x']}, {v['y']})", fontsize=8)

        # --- Graficar contorno convexo ---
        if len(contorno) >= 3:
            x_cont = [p["x"] for p in contorno] + [contorno[0]["x"]]
            y_cont = [p["y"] for p in contorno] + [contorno[0]["y"]]

            ax.plot(x_cont, y_cont, linestyle='-', linewidth=2, label="Contorno Convexo")
            ax.fill(x_cont, y_cont, alpha=0.25)

        # --- Estética ---
        ax.set_title("Restricciones, Vértices y Contorno Convexo")
        ax.set_xlabel("X")
        ax.set_ylabel("Y")
        ax.grid(True)
        ax.legend()
